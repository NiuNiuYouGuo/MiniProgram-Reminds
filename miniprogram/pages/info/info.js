...
Page({
  ...
  onLoad: function(options) {
    ...
    bwlCollection.doc(options.id).get({
      success: res => {
  ...
  cloudpreviewImage: function(event) {
    wx.previewImage({
      current: event.currentTarget.id,
      urls: this.data.bwl.imagePath
    })
  },
  cloudpreviewImagedelete: function(event) {
    let that = this
    wx.showModal({
      title: '是否清除所有图片',
      content: '图片一旦清除无法找回',
      ...
      success: function(res) {
        if (res.confirm) {
          bwlCollection.doc(detailid).update({
            data: {
              imagePath: _.remove()
            }
          }).then(success => {
            bwlCollection.doc(detailid).get().then(res => {
              that.setData({
                bwl: res.data
              })
            })
  ...
  isDoneSwitch: function(event) {
    this.setData({
      isDoneSwitch: event.detail.value
    })
  },
  add: function(event) {
    this.setData({
